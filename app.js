...
app.post('/curriculo', async (req, res) => {
  try {
    const { nome, objetivo, formacao, experiencia } = req.body;
    const newCurriculo = await pool.query(
      'INSERT INTO curriculo (nome, objetivo, formacao, experiencia) VALUES ($1, $2, $3, $4) RETURNING *',
      [nome, objetivo, formacao, experiencia]
    );
